...
# DIRECTOR OVERRIDE — the automatic remix put both verdict colours into
# decorative roles (green as the accent, red as muted body text). In this product
# red and green ARE the verdict; spending them on chrome would mean the one RED
# and the one GREEN in the film land soft. So they are removed from the palette
# entirely and reserved for the footage, which supplies them where it should.
#
# The preset's two-register system (dark ground / loud accent environment) is
# repainted as Kane Loop's OWN two registers: ink #08090C and paper #F4F5F7 —
# the exact pair the landing page flips between when the bug is present. The
# film therefore has no colour of its own. Only the product's verdicts do.
colors:
  ink-black: "#F3F5F8"          # ice — type on the ink register
  ink-black-alt: "#C9CFD8"
  fire-orange: "#F4F5F7"        # the SECOND REGISTER: paper, not a hue
  cream: "#08090C"              # ink — the ground
  cream-muted: "#6B7585"        # dim — labels, meta, secondary type
  cream-hint: "#3A414D"
  border-dark: "#242A34"        # line — the hairline
  ink-on-orange-muted: "rgba(8,9,12,0.72)"
  ink-on-orange-hint: "rgba(8,9,12,0.52)"
  ink-on-orange-faint: "rgba(8,9,12,0.34)"
  ink-on-orange-border: "rgba(8,9,12,0.14)"

# Reserved — usable ONLY where the product itself renders a verdict, and never
# as colour alone (always with the word and the glyph, as the product does):
verdict:
  fail: "#FF3B52"
  pass: "#34E39B"
...
typography:
  ...
  stat-value:{ fontFamily: "Archivo", cqw: 5.5, weight: 800, lineHeight: 1.0, tracking: "-0.04em" }
  h1:      { fontFamily: "Archivo", cqw: 7.5, weight: 800, lineHeight: 0.9, tracking: "-0.03em", lower: true }
  fadelist-item:{ fontFamily: "Archivo", cqw: 7.5, weight: 800, lineHeight: 1.0, tracking: "-0.03em", lower: true }
  quote-mark:{ fontFamily: "Archivo", cqw: 10.0, weight: 800, lineHeight: 0.6 }
  fadelist-title:{ fontFamily: "Archivo", cqw: 10.5, weight: 800, lineHeight: 0.9, tracking: "-0.04em", lower: true }
  display: { fontFamily: "Archivo", cqw: 13.0, weight: 800, lineHeight: 0.88, tracking: "-0.04em", lower: true }
...
- **Fonts** — already set to **Archivo** (display) / **JetBrains Mono** (body); ignore any preset font name lingering in prose.
- **Weights** — the brand font ships `{400, 441, 500, 600, 658, 700, 800}` only; every weight is clamped to these — ignore higher preset weights (e.g. 600/700) in prose.
...
- **The film has no colour of its own** — the second register is paper (#F4F5F7), not a hue. Red and green appear only inside the product footage, as verdicts.
...
- **Restraint** — **one register per frame**; **no colour outside the footage** (paper is a register, not an accent); one display moment; bullets capped at three.
...
- Never uppercase Archivo display. Never introduce a colour: red and green belong to the verdicts, and everything else is ink, paper, ice, dim, line.
